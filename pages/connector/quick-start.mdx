## Building a DApp to interact with the BSAFE connector

You can check this same [project](https://github.com/infinitybase/bsafe-dapp). <br/>
To create a dApp that interacts with BSAFE, you'll need beforehand:
...
- **An account and a vault created in the [BSAFE](https://app.bsafe.pro/)**
...
- @bsafe: the BSAFE SDK for interacting with resources
...
In the file `src/App.tsx` We add the Fuels provider with BSAFE connector.
...
When the user connects to the fuel wallet, they choose a predicate previously created in the BSAFE app,
...
When sending the transaction, it isn't immediately broadcasted to the network. Instead, it's sent to the BSAFE servers and waits for the vault subscribers who generated the transaction to sign it. Once all signature requirements are met, the transaction is broadcasted to the network.
...
Initially, let's create a simple contract to interact with BSAFE. To do this, create a folder called `sway`, Inside it, create a file `src/main.sw`:
...
In the BSAFE app, the vault subscribers will also receive a notification, requesting their signature.
